...
export const AtomSelectArchivedNotes = atom<null, Note>(
  null,
  (get, set, payload) => {
...
export const AtomRemoveFromArchive = atom<null, Note>(
  null,
  (get, set, note) => {
    const oldArchived = get(AtomSelectedArchivedNotes)
    const selectedArchived = oldArchived.filter(
      (tr) => tr.note_id !== note.note_id
    )
    set(AtomSelectedArchivedNotes, selectedArchived)
  }
)
